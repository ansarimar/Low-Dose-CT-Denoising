...
This code uses perceptual loss and mse for optimization. also shows the metric psnr
...
model_edge_p_mse = Model(inputs=[inputs], outputs=[outputs,outputs])
...
model_edge_p_mse.compile(optimizer=ADAM,loss=loss, loss_weights=loss_weights,metrics=[PSNRLoss])
hist_adam = model_edge_p_mse.fit(x=data,y=[labels_3,labels_3],batch_size=batch_size,epochs=20
...
#model_edge_p_mse.save('DRL_edge_p_mse70.h5')
###Test
model_edge_p_mse.load_weights('Weights/weights_DRL_edge4d_adam2_perceptual70_mse370_pig.h5')
...
[labels_pred,labels_pred]= model_edge_p_mse.predict(data_test,batch_size=8,verbose=1)
...
loss=model_edge_p_mse.evaluate(x=data_test,y=[labels_test_3,labels_test_3], batch_size=8,verbose=1)
...
#            'C://Users/mansari/Desktop/Dataset/Piglet/Prediction/Test_Low')
#write_dicom(ref ,labels_test*4095+1024,
#            'C://Users/mansari/Desktop/Dataset/Piglet/Prediction/Test_High')
#write_dicom(ref ,labels_pred*4095+1024,
#            'C://Users/mansari/Desktop/Dataset/Piglet/Prediction/DRL-Edge-Perceptual-Mse70')
##
#
#
#
...
trainable_count = int(
    np.sum([K.count_params(p) for p in set(model_edge_p.trainable_weights)]))